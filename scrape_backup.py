...
for stock in stocks:
  ...
  curr_request = fetcher.HttpAsyncRequest(curr_url)
  stock_requests.append({ "curr": curr_request })

try:
  for curr_request in curr_requests:
    if curr_request.is_complete():
      curr_html = curr_request.get_response_content("t")
      ...
      if curr_price < price_max:
        for date_i in range(len(expire_dates)):
          ...
          trows = []
          try:
            request = fetcher.HttpAsyncRequest(url)
